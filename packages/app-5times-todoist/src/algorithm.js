export const timesIntervalEnum = {
    1: 0,
    2: 1,
    3: 4,
    4: 7,
    5: 14,
};

export const algorithmData = {
    '2020-09-07': [
        {
            title: '加一',
            linkUrl: 'https://leetcode-cn.com/problems/plus-one/',
        },
    ],
    '2020-09-08': [
        {
            title: '移动零',
            linkUrl: 'https://leetcode-cn.com/problems/move-zeroes/',
        },
    ],
    '2020-09-09': [
        {
            title: '两数之和',
            linkUrl: 'https://leetcode-cn.com/problems/two-sum/',
        },
    ],
    '2020-09-10': [
        {
            title: '两两交换链表中的节点',
            linkUrl: 'https://leetcode-cn.com/problems/swap-nodes-in-pairs/',
        },
    ],
    '2020-09-11': [
        {
            title: '合并两个有序链表',
            linkUrl: 'https://leetcode-cn.com/problems/merge-two-sorted-lists/',
        },
    ],
    '2020-09-12': [
        {
            title: '猜数字游戏',
            linkUrl: 'https://leetcode-cn.com/problems/bulls-and-cows/',
        },
    ],
    '2020-09-13': [
        {
            title: '设计循环双端队列',
            linkUrl: 'https://leetcode-cn.com/problems/design-circular-deque/',
        },
    ],
    '2020-09-14': [
        {
            title: '两个数组的交集 II',
            linkUrl: 'https://leetcode-cn.com/problems/intersection-of-two-arrays-ii/',
        },
        {
            title: 'hashmaps',
            linkUrl:
                'https://adrianmejia.com/data-structures-time-complexity-for-beginners-arrays-hashmaps-linked-lists-stacks-queues-tutorial/',
        },
        {
            title: 'Map',
            linkUrl: 'https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map',
        },
    ],
    '2020-09-15': [
        {
            title: '有效的数独 36_valid-sudoku',
            linkUrl: 'https://leetcode-cn.com/problems/valid-sudoku/',
        },
        {
            title: '滑动窗口的最大值 1670_hua-dong-chuang-kou-de-zui-da-zhi-lcof',
            linkUrl: 'https://leetcode-cn.com/problems/hua-dong-chuang-kou-de-zui-da-zhi-lcof/',
        },
    ],
};

// TODO: 后续完善
// algorithmData = {
//     '2020-09-07': [{
//         title: '加一',
//         linkUrl: 'https://leetcode-cn.com/problems/plus-one/',
//         // times: 1,
//         // doneList: [],
//         // overdue: null,
//         // lastData: null
//     }]
// }
